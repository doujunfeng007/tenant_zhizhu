...
public class WfInstanceServiceImpl extends FlowServiceFactory implements IWfInstanceService {
    ...
    public void stopProcessInstance(WfTaskBo vo) {
		...
		taskService.delegateTask(taskId, vo.getComment());
    }
    ...
    public Map<String, Object> queryDetailProcess(String procInsId, String deployId) {
        ...
        if (StringUtils.isNotBlank(procInsId)) {
            ...
            taskInstanceList.forEach(taskInstance -> {
                ...
                taskVo.setProcDefKey(taskInstance.getTaskDefinitionKey());
                taskVo.setProcInsId(taskInstance.getProcessInstanceId());
                taskVo.setHisProcInsId(taskInstance.getId());
                taskVo.setProcDefName(taskInstance.getName());
                taskVo.setApplicationId((String) taskInstance.getProcessVariables().get(TaskConstants.APPLICATION_ID));
                taskVo.setTenantId(taskInstance.getTenantId());
                taskVo.setProcVars(taskInstance.getProcessVariables());
                taskVo.setTaskLocalVars(taskInstance.getTaskLocalVariables());
                ...
                if (StringUtils.isNotBlank(taskInstance.getAssignee())) {
                    User user = UserCache.getUser(TaskUtil.getUserId(taskInstance.getAssignee()));
                ...
                for (HistoricIdentityLink identityLink : linksForTask) {
                    if ("candidate".equals(identityLink.getType())) {
                        if (StringUtils.isNotBlank(identityLink.getUserId())) {
                            User user = UserCache.getUser(Long.parseLong(identityLink.getUserId()));
                            if(ObjectUtil.isNotEmpty(user)){
