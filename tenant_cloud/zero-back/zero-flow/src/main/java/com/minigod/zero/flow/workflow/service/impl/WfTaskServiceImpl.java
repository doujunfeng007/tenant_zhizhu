...
import com.minigod.zero.flow.core.common.constant.ProcessConstants;
...
import com.minigod.zero.flow.flowable.flowable.flow.CustomProcessDiagramGenerator;
import com.minigod.zero.flow.flowable.flowable.flow.FindNextNodeUtil;
...
import com.minigod.zero.flow.workflow.domain.vo.WfViewerVo;
...
public class WfTaskServiceImpl extends FlowServiceFactory implements IWfTaskService {
	...
	public void claim(WfTaskBo taskBo) {
		...
		taskService.claim(taskBo.getTaskId(), String.valueOf(AuthUtil.getUserId()));
	}
	...
	public void delegateTask(WfTaskBo bo) {
		...
		StringBuilder commentBuilder = new StringBuilder(AuthUtil.getUser().getUserName()).append("->");
		User user = UserCache.getUser(Long.parseLong(bo.getUserId()));
		if (ObjectUtil.isNotEmpty(user)) {
			commentBuilder.append(user.getRealName());
		} else {
			commentBuilder.append(bo.getUserId());
		}
		if (StringUtil.isNotBlank(bo.getComment())) {
		...
		// 设置办理人为当前登录人
		taskService.setOwner(bo.getTaskId(), AuthUtil.getUser().getUserId().toString());
		// 执行委派
	...
	public void transferTask(WfTaskBo bo) {
		...
		String applicationId = String.valueOf(map.get(TaskConstants.APPLICATION_ID));
		String tableName = String.valueOf(map.get(TaskConstants.APPLICATION_TABLE));
		FlowTable flowTable = new FlowTable();
		flowTable.setApplicationId(applicationId);
		flowTable.setTableName(tableName);
		flowTable.setAssignDrafter(bo.getUserId());
		flowTableService.updateAssign(flowTable);
		// 设置任务节点名称
		bo.setTaskName(task.getName());
		// 处理抄送用户
		if (!copyService.makeCopy(bo)) {
	...
	public void stopProcess(WfTaskBo bo) {
		...
		if (Objects.nonNull(bpmnModel)) {
			...
			if (CollectionUtil.isNotEmpty(endNodes)) {
				Authentication.setAuthenticatedUserId(AuthUtil.getUser().getUserId().toString());
...
//                        StringUtils.isBlank(flowTaskVo.getComment()) ? "取消申请" : flowTaskVo.getComment());
	...
	public void revokeProcess(WfTaskBo bo) {
		...
		for (HistoricTaskInstance hti : htiList) {
			if (AuthUtil.getUser().getUserId().toString().equals(hti.getAssignee())) {
				myTaskId = hti.getId();
				myTask = hti;
				break;
			}
...
//      Map<String, VariableInstance> variables = runtimeService.getVariableInstances(currentTask.getExecutionId());
	...
	public WfViewerVo getFlowViewer(String procInsId) {
		...
		List<HistoricActivityInstance> finishedElementList = allActivityInstanceList.stream()
			.filter(item -> ObjectUtil.isNotEmpty(item.getEndTime())).collect(Collectors.toList());
		// 所有已完成的连线
		...
		Set<String> unfinishedTaskSet = allActivityInstanceList.stream()
			.filter(item -> ObjectUtil.isEmpty(item.getEndTime()))
	...
	public void startFirstTask(ProcessInstance processInstance, Map<String, Object> variables) {
		...
		if (Objects.nonNull(task)) {
			...
			taskService.addComment(task.getId(), processInstance.getProcessInstanceId(), FlowComment.NORMAL.getType(), "工作流启动");
