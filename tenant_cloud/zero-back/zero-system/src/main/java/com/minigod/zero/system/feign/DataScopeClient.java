...
import com.minigod.zero.core.datascope.constant.DataScopeConstant;
...
public class DataScopeClient implements IDataScopeClient {
	...
	public DataScopeModel getDataScopeByCode(String code) {
		...
		// 后续若有新增配置则会清空缓存重新加载
		DataScopeModel dataScope;
		List<DataScopeModel> list = jdbcTemplate.query(DataScopeConstant.DATA_BY_CODE, new BeanPropertyRowMapper<>(DataScopeModel.class), new Object[]{code});
		if (CollectionUtil.isNotEmpty(list)) {
			dataScope = list.iterator().next();
			dataScope.setSearched(Boolean.TRUE);
		} else {
	...
	public List<Long> getDeptAncestors(Long deptId) {
		return jdbcTemplate.queryForList(DataScopeConstant.DATA_BY_DEPT, Long.class, new Object[]{deptId});
	}
}
