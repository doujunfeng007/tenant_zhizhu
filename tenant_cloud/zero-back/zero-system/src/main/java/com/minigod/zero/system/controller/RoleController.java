...
import com.minigod.zero.system.wrapper.RoleWrapper;
...
public class RoleController extends ZeroController {
	...
	public R<List<RoleVO>> list(@ApiIgnore @RequestParam Map<String, Object> role, ZeroUser zeroUser) {
		QueryWrapper<Role> queryWrapper = Condition.getQueryWrapper(role, Role.class);
		List<Role> list = roleService.list((!zeroUser.getTenantId().equals(ZeroConstant.ADMIN_TENANT_ID)) ? queryWrapper.lambda().eq(Role::getTenantId, zeroUser.getTenantId()) : queryWrapper);
	...
	public R<List<RoleVO>> tree(String tenantId, ZeroUser zeroUser) {
		List<RoleVO> tree = roleService.tree(Func.toStrWithEmpty(tenantId, zeroUser.getTenantId()));
		return R.data(tree);
	}
	...
	@ApiOperation(value = "树形结构", notes = "树形结构")
	public R<List<RoleVO>> treeById(Long roleId, ZeroUser zeroUser) {
		Role role = SysCache.getRole(roleId);
		List<RoleVO> tree = roleService.tree(Func.notNull(role) ? role.getTenantId() : zeroUser.getTenantId());
		return R.data(tree);
	}
