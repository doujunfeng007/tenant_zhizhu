...
import com.minigod.zero.system.wrapper.RegionWrapper;
...
public class RegionController extends ZeroController {
	...
	public R importRegion(MultipartFile file, Integer isCovered) {
		try {
			List<RegionExcel> list = ExcelUtil.readWebExcelData(file,RegionExcel.class);
			regionService.importRegion(list,isCovered == 1);
		}catch (Exception e){
