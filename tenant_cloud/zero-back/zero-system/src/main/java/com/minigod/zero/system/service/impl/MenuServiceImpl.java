...
import com.minigod.zero.system.mapper.MenuMapper;
import com.minigod.zero.system.mapper.TopMenuMapper;
import com.minigod.zero.system.service.IMenuService;
import com.minigod.zero.system.service.IRoleMenuService;
import com.minigod.zero.system.service.IRoleScopeService;
...
import static com.minigod.zero.common.constant.CommonConstant.API_SCOPE_CATEGORY;
import static com.minigod.zero.common.constant.CommonConstant.DATA_SCOPE_CATEGORY;
...
public class MenuServiceImpl extends ServiceImpl<MenuMapper, Menu> implements IMenuService {
	...
	@Override
	public List<MenuVO> tenantMenuList(Long parentId, Map<String, Object> param) {
		if (Func.isEmpty(Func.toStr(param.get(PARENT_ID)))) {
			parentId = null;
		}
		return baseMapper.tenantMenuList(parentId, param);
	}
	...
	@Override
	public List<TreeNode> grantTree(ZeroUser user) {
		List<TreeNode> menuTree = user.getTenantId().equals(ZeroConstant.ADMIN_TENANT_ID) ? baseMapper.grantTree() : baseMapper.grantTreeByRole(Func.toLongList(user.getRoleId()));
		return ForestNodeMerger.merge(tenantPackageTree(menuTree, user.getTenantId()));
	}

	@Override
	public List<TreeNode> tenantGrantTree(ZeroUser user) {
		List<TreeNode> menuTree = user.getTenantId().equals(ZeroConstant.ADMIN_TENANT_ID) ? baseMapper.tenantGrantTree() : baseMapper.tenantGrantTreeByRole(Func.toLongList(user.getRoleId()));
		List<TreeNode> menuTreeList = ForestNodeMerger.merge(tenantPackageTree(menuTree, user.getTenantId()));
		if (!CollectionUtil.isEmpty(menuTreeList)){
			List<TreeNode> topMenuTreeList = new ArrayList<>();
			for (TreeNode treeNode : menuTreeList){
				TopMenu topMenu = topMenuMapper.selectByMenuId(treeNode.getId());
				if (topMenu == null){
					topMenuTreeList.add(treeNode);
					continue;
				}
				Integer topNodeIndex = getToNode(topMenuTreeList,topMenu.getId());
				if (topNodeIndex == null){
					TreeNode topNode = new TreeNode();
					topNode.setId(topMenu.getId());
					topNode.setTitle(topMenu.getName());
					topNode.setValue(topMenu.getId());
					...
					topMenuTreeList.add(topNode);
				}else{
					TreeNode node = topMenuTreeList.get(topNodeIndex);
					List<TreeNode> childrenList = node.getChildren();
					if (CollectionUtil.isEmpty(childrenList)){
						childrenList = new ArrayList<>();
					}
					childrenList.add(treeNode);
					topMenuTreeList.set(topNodeIndex,node);
				}
			}
			return topMenuTreeList;
		}
		return menuTreeList;
	}

	private Integer getToNode(List<TreeNode> topMenuTreeList,Long topMenuId){
		for (int i = 0 ;i<topMenuTreeList.size();i++){
			if (topMenuTreeList.get(i).getId().equals(topMenuId)){
				return i;
			}
		}
		return null;
	...
	@Override
	public List<TreeNode> grantTopTree(ZeroUser user) {
		List<TreeNode> menuTree = user.getTenantId().equals(ZeroConstant.ADMIN_TENANT_ID) ? baseMapper.grantTopTree() : baseMapper.grantTopTreeByRole(Func.toLongList(user.getRoleId()));
		return ForestNodeMerger.merge(tenantPackageTree(menuTree, user.getTenantId()));
	}
	...
	private List<Menu> tenantPackageMenu(List<Menu> menu) {
		log.info("获取到菜单列表:{}", Collections.singletonList(menu));
		TenantPackage tenantPackage = SysCache.getTenantPackage(AuthUtil.getTenantId());
		log.info("获取到租户包:{}", tenantPackage);
		if (Func.isNotEmpty(tenantPackage) && tenantPackage.getId() > 0L) {
			...
			log.info("比对后结果:{}", List.of(menu));
		}
		return menu;
	}

	@Override
	public List<TreeNode> grantDataScopeTree(ZeroUser user) {
		return ForestNodeMerger.merge(user.getTenantId().equals(ZeroConstant.ADMIN_TENANT_ID) ? baseMapper.grantDataScopeTree() : baseMapper.grantDataScopeTreeByRole(Func.toLongList(user.getRoleId())));
	}

	@Override
	public List<TreeNode> grantApiScopeTree(ZeroUser user) {
		return ForestNodeMerger.merge(user.getTenantId().equals(ZeroConstant.ADMIN_TENANT_ID) ? baseMapper.grantApiScopeTree() : baseMapper.grantApiScopeTreeByRole(Func.toLongList(user.getRoleId())));
	}
	...
	public List<Kv> authRoutes(ZeroUser user) {
	...
	public boolean submit(Menu menu) {
		...
		if (menu.getParentId() == null && menu.getId() == null) {
			menu.setParentId(ZeroConstant.TOP_PARENT_ID);
		}
		if (menu.getParentId() != null && menu.getId() == null) {
		...
		menu.setIsDeleted(ZeroConstant.DB_NOT_DELETED);
