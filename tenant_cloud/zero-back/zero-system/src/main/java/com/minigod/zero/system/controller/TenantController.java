...
import static com.minigod.zero.core.tenant.constant.TenantBaseConstant.TENANT_DATASOURCE_CACHE;
import static com.minigod.zero.core.tenant.constant.TenantBaseConstant.TENANT_DATASOURCE_EXIST_KEY;
...
public class TenantController extends ZeroController {
	...
	@PreAuth(RoleConstant.HAS_ROLE_ADMIN)
	public R<IPage<Tenant>> list(@ApiIgnore @RequestParam Map<String, Object> tenant, Query query, ZeroUser zeroUser) {
		QueryWrapper<Tenant> queryWrapper = Condition.getQueryWrapper(tenant, Tenant.class);
		queryWrapper.orderByDesc("create_time");
		IPage<Tenant> pages = tenantService.page(Condition.getPage(query),
			(!zeroUser.getTenantId().equals(ZeroConstant.ADMIN_TENANT_ID))
				? queryWrapper.lambda().eq(Tenant::getTenantId, zeroUser.getTenantId()) : queryWrapper);
		return R.data(pages);
	}
	...
	@PreAuth(RoleConstant.HAS_ROLE_ADMIN)
	public R<List<Tenant>> select(Tenant tenant, ZeroUser zeroUser) {
		QueryWrapper<Tenant> queryWrapper = Condition.getQueryWrapper(tenant);
		List<Tenant> list = tenantService.list((!zeroUser.getTenantId().equals(ZeroConstant.ADMIN_TENANT_ID)) ? queryWrapper.lambda().eq(Tenant::getTenantId, zeroUser.getTenantId()) : queryWrapper);
		return R.data(list);
	}
	...
	public R packageSetting(@ApiParam(value = "租户ID", required = true) @RequestParam String tenantId, @ApiParam(value = "产品包ID") Long packageId) {
		CacheUtil.evict(SYS_CACHE, SysCache.TENANT_TENANT_ID, tenantId, Boolean.FALSE);
		CacheUtil.evict(SYS_CACHE, SysCache.TENANT_PACKAGE_ID, tenantId, Boolean.FALSE);
	...
	@ApiOperation(value = "终端列表", notes = "传入tenant")
	public R tenantList(Tenant tenant) {
		return R.data(tenantService.list(Condition.getQueryWrapper(tenant)));
	}
}
