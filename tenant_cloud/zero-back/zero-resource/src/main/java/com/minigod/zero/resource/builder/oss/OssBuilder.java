...
import com.minigod.zero.core.oss.enums.OssStatusEnum;
import com.minigod.zero.core.oss.props.OssProperties;
import com.minigod.zero.core.oss.rule.OssRule;
import com.minigod.zero.core.oss.rule.ZeroOssRule;
...
public class OssBuilder {
	...
	public OssTemplate template(String code) {
		Oss oss = getOssNew(tenantId, code);
        log.info("查询出的oss对象信息为==={},租户id=={},code=={}", JSONUtil.toJsonStr(oss),tenantId,code);
		Oss ossCached = ossPool.get(tenantId);
		log.info("查询出的ossCached对象信息为==={}", JSONUtil.toJsonStr(ossCached));
			...
			synchronized (OssBuilder.class) {
				...
				if (Func.hasEmpty(template, ossCached) || !oss.getEndpoint().equals(ossCached.getEndpoint()) || !oss.getAccessKey().equals(ossCached.getAccessKey())) {
					...
					if (oss.getEndpoint().equals(ossProperties.getEndpoint()) && oss.getAccessKey().equals(ossProperties.getAccessKey()) && ossProperties.getTenantMode()) {
						ossRule = new ZeroOssRule(Boolean.TRUE);
					} else {
						ossRule = new ZeroOssRule(Boolean.FALSE);
					}
		...
		log.info("查询出的template对象信息为==={}", JSONUtil.toJsonStr(template));
		return template;
	}
	...
	public Oss getOss(String tenantId, String code) {
		...
		String ossCode = StringUtil.isBlank(code) ? OssEnum.QINIU.getName() : code;
	...
	public Oss getOssNew(String tenantId, String code) {
		LambdaQueryWrapper<Oss> lqw = Wrappers.<Oss>query().lambda().eq(Oss::getTenantId, tenantId);
		// 获取传参的资源编号并查询，若有则返回，若没有则调启用的配置
		String ossCode = StringUtil.isBlank(code) ? OssEnum.MINIO.getName() : code;
		if (StringUtil.isNotBlank(ossCode)) {
			lqw.eq(Oss::getOssCode, ossCode);
		} else {
			lqw.eq(Oss::getStatus, OssStatusEnum.ENABLE.getNum());
		...
		if ((Func.isEmpty(o))) {
			...
			log.info("oss:"+JSONUtil.toJsonStr(o));
			return o;
		}
	}
