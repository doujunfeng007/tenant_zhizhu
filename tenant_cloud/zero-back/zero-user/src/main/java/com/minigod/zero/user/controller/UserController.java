...
@NonDS
@RestController
@RequestMapping(AppConstant.BACK_API_PREFIX + "/user")
@AllArgsConstructor
...
public class UserController {
	...
	@ApiOperation(value = "查看详情", notes = "传入id")
	@GetMapping("/info")
	public R<UserVO> info(ZeroUser user) {
	...
	@PreAuth(RoleConstant.HAS_ROLE_ADMIN)
	public R<IPage<UserVO>> list(@ApiIgnore @RequestParam Map<String, Object> user, Query query, ZeroUser zeroUser) {
		QueryWrapper<User> queryWrapper = Condition.getQueryWrapper(user, User.class);
		IPage<User> pages = userService.page(Condition.getPage(query), (!zeroUser.getTenantId().equals(ZeroConstant.ADMIN_TENANT_ID)) ? queryWrapper.lambda().eq(User::getTenantId, zeroUser.getTenantId()) : queryWrapper);
	...
	@ApiOperation(value = "列表", notes = "传入account和realName")
	@PreAuth(RoleConstant.HAS_ROLE_ADMIN)
	public R<IPage<UserVO>> page(@ApiIgnore User user, Query query, Long deptId, ZeroUser zeroUser) {
		String tenantId= "000000";
		if (zeroUser != null && zeroUser.getTenantId() != null) {
			if(tenantId.equals(zeroUser.getTenantId())){
				tenantId="";
			}else{
				tenantId = zeroUser.getTenantId();
			}
		}
		IPage<User> pages = userService.selectUserPage(Condition.getPage(query), user, deptId, tenantId);
		return R.data(UserWrapper.build().pageVO(pages));
	}
	...
	public R submit(@Valid @RequestBody User user) {
		if (user == null || StringUtils.isAnyBlank(user.getPhone(), user.getEmail(), user.getRealName())) {
			return R.fail("手机号、邮箱、姓名都不能为空！");
		}
		if (user.getPhone().contains("-")) {
			String[] split = user.getPhone().split("-");
			user.setPhone(split[1]);
			user.setArea(split[0]);
		}else {
			return R.fail("手机号需要带上区号！如86-13812345678");
		}
	...
	public R updatePassword(ZeroUser user, @ApiParam(value = "旧密码", required = true) @RequestParam String oldPassword,
	...
	public R<List<User>> userList(User user, ZeroUser zeroUser) {
		QueryWrapper<User> queryWrapper = Condition.getQueryWrapper(user);
			//.orderBy(true, true, "user_type").orderBy(true, false, "update_time");
		List<User> list = userService.list((!AuthUtil.isAdministrator()) ? queryWrapper.lambda().eq(User::getTenantId, zeroUser.getTenantId()) : queryWrapper);
		return R.data(list);
	}
	...
	public R importUser(MultipartFile file, Integer isCovered) {
		try{
			List<UserExcel> list = ExcelUtil.readWebExcelData(file,UserExcel.class);
			userService.importUser(list,isCovered == 1);
		}catch (Exception e){
	...
	public void exportUser(@ApiIgnore @RequestParam Map<String, Object> user, ZeroUser zeroUser, HttpServletResponse response) {
		QueryWrapper<User> queryWrapper = Condition.getQueryWrapper(user, User.class);
		if (!AuthUtil.isAdministrator()) {
			queryWrapper.lambda().eq(User::getTenantId, zeroUser.getTenantId());
		}
		queryWrapper.lambda().eq(User::getIsDeleted, ZeroConstant.DB_NOT_DELETED);
	...
	public R updatePlatform(Long userId, Integer userType, String userExt) {
		return R.status(userService.updatePlatform(userId, userExt));
	}
	...
	public R unlock(String userIds) {
		...
		userList.forEach(user -> zeroRedis.del(CacheNames.tenantKey(user.getTenantId(), CacheNames.USER_FAIL_KEY, user.getAccount())));
		return R.success("操作成功");
	}
