...
//		return CacheUtil.get(SYS_CACHE, TENANT_PACKAGE_ID, tenantId, () -> {
		...
		TenantPackage data = result.getData();
...
//		}, Boolean.FALSE);
	...
	public static List<FuncConfigVO> getFuncConfigByTenantId(String tenantId) {
		return CacheUtil.get(SYS_CACHE, "funcConfig:tenantId:", tenantId, () -> {
			R<List<FuncConfigVO>> result = getSysClient().getFuncConfig(tenantId);
			return result.getData();
		}, Boolean.FALSE);
	}
}
