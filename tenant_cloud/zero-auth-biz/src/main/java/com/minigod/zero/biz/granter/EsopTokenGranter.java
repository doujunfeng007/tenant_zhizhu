 ...
 * @author Chill
 */
public class EsopTokenGranter extends AbstractTokenGranter {

	private static final String GRANT_TYPE = "esop_acct";

	private ICustAuthClient custAuthClient;

	private UserDetailsService userDetailsService;

	protected EsopTokenGranter(UserDetailsService userDetailsService, AuthorizationServerTokenServices tokenServices, ClientDetailsService clientDetailsService,
                               OAuth2RequestFactory requestFactory, ICustAuthClient custAuthClient) {
	...
	protected OAuth2Authentication getOAuth2Authentication(ClientDetails client, TokenRequest tokenRequest) {
		...
		R<Object> result = custAuthClient.esopLogin(custRegister);// 校验密码
		if (!result.isSuccess()) {
