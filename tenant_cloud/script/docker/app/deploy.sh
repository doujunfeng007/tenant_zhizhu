...
#启动基础模块
base(){
	docker-compose up -d nacos sentinel seata-server web-nginx zero-nginx zero-redis
}

#启动监控模块
monitor(){
	docker-compose up -d zero-admin zero-turbine
}

#启动程序模块
modules(){
	docker-compose up -d zero-gateway1 zero-gateway2 zero-auth1 zero-auth2 zero-swagger zero-report zero-user zero-desk zero-system zero-log zero-flow zero-resource
}
