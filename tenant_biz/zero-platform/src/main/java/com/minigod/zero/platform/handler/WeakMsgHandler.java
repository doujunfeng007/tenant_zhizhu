...
public class WeakMsgHandler {
	...
	public R weakMsgHandler(int devType,  List<Long> lstUserIds, String notificationExtras, boolean isOfflinePush, int intPushType, String title, Set<Long> succUsers,List<CustomerDeviceInfoVO> deviceList) {
		...
		if (!CollectionUtils.isEmpty(newDeviceList)) {
			...
			if(CollectionUtils.isNotEmpty(androidDeviceList)){
			...
			if(CollectionUtils.isNotEmpty(iosDeviceList)){
				Set<String> lstIosDeviceIdSet = iosDeviceList.stream().map(CustomerDeviceInfoVO::getOpenCode).filter(org.apache.commons.lang.StringUtils::isNotBlank).collect(Collectors.toSet());
				Set<Long> lstIosUserIdSet = iosDeviceList.stream().filter(next -> org.apache.commons.lang.StringUtils.isNotBlank(next.getOpenCode())).map(CustomerDeviceInfoVO::getCustId).collect(Collectors.toSet());
	...
	private void jPush(int osType, boolean isOfflinePush, int intPushType, String title, Set<Long> succUsers,
						 Set<String> lstDeviceIdSet, Set<Long> lstUserIdSet,
		...
		try {
			...
			jPushDTO.setPushType(intPushType);
			...
			log.error("推送消息给{}用户异常", PlatformOsTypeEnum.getTypeName(osType), e);
		}
	}
}
