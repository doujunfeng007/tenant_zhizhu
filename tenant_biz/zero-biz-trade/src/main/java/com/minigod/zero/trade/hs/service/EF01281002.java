...
import com.minigod.zero.trade.hs.resp.FundAccountInfoWithCash;
...
public class EF01281002 extends Abstractbiz {
    @Resource
    ...
    protected <T extends GrmRequestVO> GrmResponseVO request(T request) {
        ...
        List<FundAccountInfo> fundAccountInfoWithCashList = new ArrayList<>();
        FundAccountInfoWithCash fundAccountInfoWithCash;
        for(Object fo :fundAccts){
            ...
            fundAccountInfoWithCash = new FundAccountInfoWithCash();
            ...
            fundAccountInfoWithCashList.add(fundAccountInfoWithCash);
        }
        //存入客户非登录session
        clientSession.setFundAccts(fundAccountInfoWithCashList);
