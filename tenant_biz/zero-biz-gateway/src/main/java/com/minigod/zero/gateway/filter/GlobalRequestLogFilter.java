...
@ConditionalOnProperty(value = "zero.log.request.enabled", havingValue = "true", matchIfMissing = true)
public class GlobalRequestLogFilter implements GlobalFilter, Ordered {
	...
	public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
		...
		headers.forEach((headerName, headerValue) -> {
			...
			if (TokenConstant.HEADER.toLowerCase().equals(headerName)) {
				String value = headerValue.get(0);
				String token = Jwt2Util.getToken(value);
