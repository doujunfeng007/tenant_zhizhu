...
import com.minigod.zero.bpmn.module.withdraw.request.BankSwiftCodeRequest;
...
public interface ICashBankClient {
	...
	R withdrawBankList(@RequestParam("bankType") Integer bankType);

	@GetMapping(QUERY_BANK_SWIFT_CODE)
	R<BankSwiftCodeRequest> bankSwiftCode(@RequestParam("bankCode") String bankCode);
	...
	R<List<CashDepositWay>> selectDepositWay(@RequestParam("currency")Integer currency, @RequestParam("bankType")Integer bankType);
}
