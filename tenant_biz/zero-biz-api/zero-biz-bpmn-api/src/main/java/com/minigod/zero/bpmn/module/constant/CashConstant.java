...
public class CashConstant {
	...
	public enum MoneyTypeDescEnum {
		...
		public static MoneyTypeDescEnum getByDesc(String desc) {
			if (StringUtil.isEmpty(desc)) {
				return null;
			}
			List<MoneyTypeDescEnum> collect = Arrays.stream(values()).filter(o -> o.getDescription().toUpperCase().equals(desc.toUpperCase())).collect(Collectors.toList());
			if (CollectionUtil.isNotEmpty(collect)) {
				return collect.get(0);
			}
			return null;
		...
		public static String getValue(String desc) {
			MoneyTypeDescEnum anEnum = getByDesc(desc);
			if (anEnum == null) {
				return null;
			} else {
	...
	// 入金表 币种 1港币 2美元 3人民币
	public enum PayCurrencyTypeDescEnum {
		CNY("3", "CNY"),
		USD("2", "USD"),
		HKD("1", "HKD")
		;
		...
		PayCurrencyTypeDescEnum(String value, String description) {
			this.value = value;
			this.description = description;
		}

		public static PayCurrencyTypeDescEnum getByCode(String value) {
			if (StringUtil.isBlank(value)) {
				return null;
			}
			List<PayCurrencyTypeDescEnum> collect = Arrays.stream(values()).filter(o -> o.getValue().equals(value)).collect(Collectors.toList());
			if (CollectionUtil.isNotEmpty(collect)) {
				return collect.get(0);
			}
			return null;
		...
		public static String getDesc(String value) {
			PayCurrencyTypeDescEnum anEnum = getByCode(value);
			if (anEnum == null) {
				return null;
			} else {
				return anEnum.getDescription();
