...
1. 访问地址：http://localhost:8100/oauth/authorize?client_id=minigod&redirect_uri=http://example.com&code=233333&response_type=code
2. 获取跳转后的code值(http://example.com/?code=VhYNLR)之后，调用 http://localhost/minigod-auth/oauth/token 传入对应的参数
...
1. 直接调用 http://localhost/minigod-auth/oauth/token 传入对应的参数
...
2. 调用 http://localhost/minigod-auth/oauth/user-info 既可获得对应用户信息