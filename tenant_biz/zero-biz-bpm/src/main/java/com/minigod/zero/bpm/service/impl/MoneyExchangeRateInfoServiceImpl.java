...
import com.minigod.zero.bpm.dto.*;
...
import com.minigod.zero.trade.feign.ICustFundHsClient;
...
//		}
		...
		customerCurrencyExchangeInfo.setCustId(AuthUtil.getCustId());
		...
		R<FinancingAccountAmount> financingAccountAmountR = iCustomerInfoClient.accountAmount(customerAccount.getAcct().getCapitalAccount(), CashConstant.MoneyTypeDescExchangeEnum.getDesc(dto.getCurrencyOut()));
		if (financingAccountAmountR.isSuccess()) {
			...
			if (i > 0) {
				...
				return R.fail(ResultCode.DISPLAY_ERROR.getCode(), "客户可提余额不足");
			}
		}else {
			log.warn("获取可提余额失败：" + custId);
			return R.fail(ResultCode.DISPLAY_ERROR.getCode(), "获取余额失败");
		}
