...
public class TimeZoneUtil {
    ...
    public static boolean isDaylightTimeInUSA(Date dateNeedCheck) {
        ...
        return startDayInSecond <= dateNeedCheckInSecond
                && dateNeedCheckInSecond <= endDayInSecond;
    }

    /**
     * 根据传入的年份获取夏令时的开始和结束时间
     * @param year
     * @return
     */
    public static TimeZoneVO getDaylightTimeInUSA(int year) {
        TimeZoneVO timeZoneVO = new TimeZoneVO();
        // 3月的第二个星期日
        Calendar daylightSavingTimeBeginDay = DateUtils.getSecondSundayOfMonth(
                year, 3);
        //夏令时第一天凌晨2点
        daylightSavingTimeBeginDay.set(Calendar.HOUR_OF_DAY, 2);
        // 11月的第一个星期日
        Calendar daylightSavingTimeEndDay = DateUtils.getFirstSundayOfMonth(
                year, 11);
        //夏令时最后一天凌晨2点
        daylightSavingTimeEndDay.set(Calendar.HOUR_OF_DAY, 1);
        timeZoneVO.setSummerStartTime(daylightSavingTimeBeginDay.getTime());
        timeZoneVO.setSummerEndTime(daylightSavingTimeEndDay.getTime());
        return timeZoneVO;
    }
