...
public class RestTemplateUtil {
	...
	public <T extends Serializable> T getSend(String url, Class<T> clazz, Map<String, String> params) {
		try {
			HttpEntity<Object> requestEntity = new HttpEntity<>(params, getHeaders(params));String paramsStr = "";
			...
			R result = restTemplate.getForObject(url, R.class, requestEntity);
			if (result == null) {
				return null;
			}
			if (result.getCode() == R.success().getCode() && result.getData() != null) {
				return DateFormatUtil.getObjectMapper().convertValue(result.getData(), clazz);
			}
			log.warn(url + result.getMsg());
		} catch (Exception e) {
			log.error(url + "远程调用异常", e);
	...
	public <T extends Serializable> List<T> getSends(String url, Class<T> clazz, Map<String, String> params) {
		try {
			...
			R<List<T>> result = restTemplate.getForObject(url, R.class, requestEntity);
			if (null == result) {
				return Collections.emptyList();
			}
			if (result.getCode() != R.success().getCode()) {
			...
			if (CollectionUtil.isNotEmpty(result.getData())) {
				List<T> list = new ArrayList<>();
				for (T data : result.getData()) {
					list.add(DateFormatUtil.getObjectMapper().convertValue(data, clazz));
				}
				return list;
			}
