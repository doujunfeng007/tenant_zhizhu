...
Table: minigod_datasource（数据源配置表）
...
Table: minigod_model_prototype（数据原型表）
...
Table: minigod_process_leave（流程请假业务表）
...
Table: minigod_report_file（报表文件表）
...
Table: minigod_role_menu（角色菜单关联表）
...
Table: minigod_role_scope（角色数据权限关联表）
...
Table: minigod_scope_data（数据权限表）
...
Table: minigod_top_menu_setting（顶部菜单配置表）
...
Table: minigod_user_oauth（用户第三方认证表）
...
Table: minigod_user_web（用户平台拓展表）
...
Table: minigod_user_app（用户平台拓展表）
...
Table: minigod_user_other（用户平台拓展表）
