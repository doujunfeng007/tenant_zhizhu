...
#启动基础模块
base(){
	docker-compose up -d nacos sentinel seata-server web-nginx minigod-nginx minigod-redis
}

#启动监控模块
monitor(){
	docker-compose up -d minigod-admin minigod-turbine
}

#启动程序模块
modules(){
	docker-compose up -d minigod-gateway1 minigod-gateway2 minigod-auth1 minigod-auth2 minigod-swagger minigod-report minigod-user minigod-desk minigod-system minigod-log minigod-flow minigod-resource
}
